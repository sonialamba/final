	<?php
	session_start();
	include "validateMenteeFeedback.php";
	require "connect.php";
...
<script src="owl_carousel/owl-carousel/owl.carousel.js"></script>
  ...
  <form method = "post" action="#">
    ...
    <select type="text" name="menteeName" id="name">
		<?php
			$sql = "select mentorinfo_id,ment_fname,ment_lname from mentor_info where mentorinfo_id in (select mentor_id from pairtable where mentee_id = ".$user_id.")";
			$result = mysqli_query($conn, $sql);
			while($row = mysqli_fetch_assoc($result))
			{
				echo "<option ";
				echo "value = ".$row["mentorinfo_id"]." >";
				echo $row["ment_fname"]." ".$row["ment_lname"];
				echo "</option>";	
			}	
		?>
	</select>
